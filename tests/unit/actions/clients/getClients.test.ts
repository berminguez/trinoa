...
describe('getClients', () => {
  ...
  it('debe obtener lista de clientes con valores por defecto', async () => {
    ...
    mockPayload.find.mockImplementation((params) => {
      ...
      if (params.collection === 'projects' && params.limit === 0) {
        return Promise.resolve({
          docs: [],
          totalDocs: 3, // Mock: user-1 tiene 3 proyectos
        })
      }
      // Mock respuesta de proyecto más reciente
      if (params.collection === 'projects' && params.limit === 1) {
        return Promise.resolve({
          docs: [
            {
              id: 'project-1',
  ...
  it('debe aplicar filtros de búsqueda correctamente', async () => {
    ...
    mockPayload.find.mockImplementation((params) => {
      ...
      // Mock contador de proyectos
      if (params.collection === 'projects' && params.limit === 0) {
        return Promise.resolve({ totalDocs: 2 })
      }
      // Mock proyecto más reciente
      if (params.collection === 'projects' && params.limit === 1) {
        return Promise.resolve({
      ...
      if (params.collection === 'resources' && params.limit === 1) {
        return Promise.resolve({
          docs: [{ id: 'r1', updatedAt: new Date().toISOString() }],
          totalDocs: 1,
        })
      }
      return Promise.resolve({ docs: [], totalDocs: 0 })
    ...
    expect(mockPayload.find).toHaveBeenCalledWith({
      ...
      where: {
        or: [
          { name: { contains: 'juan' } },
          { email: { contains: 'juan' } },
          { filial: { contains: 'juan' } },
          { 'empresa.name': { contains: 'juan' } },
        ],
        role: { equals: 'user' },
      },
  ...
  it('debe calcular estadísticas de proyectos para cada cliente', async () => {
    ...
    mockPayload.find.mockImplementation((params) => {
      ...
      // Mock contador de proyectos por cliente
      if (params.collection === 'projects' && params.limit === 0) {
      ...
      if (params.collection === 'projects' && params.limit === 1) {
        return Promise.resolve({
          docs: [
            {
              id: 'project-recent',
              updatedAt: new Date().toISOString(), // Actividad reciente
            },
          ],
          totalDocs: 1,
        })
